...
export const NotifProvider = ({ children }: { children: ReactNode }) => {
    ...
    // 1. Chargement des notifications initiales + temps réel
    useEffect(() => {
        if (!user) {
            setNotifications([]);
            return;
    ...
    // 2. Vérification des rappels programmés (Run once per session/day to avoid lag)
    useEffect(() => {
        if (!user || user.role !== 'commercial') return;

        const checkScheduledReminders = async () => {
            console.log('[NotifProvider] Checking scheduled reminders...');
            const now = new Date();
            ...
            for (const d of datesToCheck) {
                // ... (Logic remains similar but isolated)
                ...
                if (visits && visits.length > 0) {
                    for (const v of visits) {
                        ...
                        const { data: existing } = await supabase.from('notifications').select('id').eq('assigned_to', user.id).eq('title', reminderTitle).limit(1);
                        if (existing && existing.length > 0) continue;

                        const { data: contact } = await supabase.from('contacts').select('name').eq('id', v.contact_id).single();
                        const prospectName = contact?.name || 'Prospect Inconnu';
                        let typeLabel = (v.type === 'terrain' ? 'visite de terrain' : v.type === 'chantier' ? 'visite de chantier' : 'rendez-vous');
                        const message = `${d.label} à ${v.heure}, vous avez une ${typeLabel} avec ${prospectName} à ${v.lieu || 'Lieu non spécifié'}.`;

                        await supabase.from('notifications').insert([{ type: 'rdv', title: reminderTitle, message, assigned_to: user.id, service: user.service }]);
                    }
                }

                const { data: followups } = await supabase.from('follow_ups').select('id, note, date_relance, contact_id').eq('agent', user.name).eq('date_relance', d.date).neq('statut', 'done');
                if (followups && followups.length > 0) {
                    for (const f of followups) {
                        ...
                        const { data: contact } = await supabase.from('contacts').select('name').eq('id', f.contact_id).single();
                        const prospectName = contact?.name || 'Prospect Inconnu';
                        const message = `${d.label}, vous devez effectuer une action de suivi pour ${prospectName} : "${f.note}".`;
                        await supabase.from('notifications').insert([{ type: 'tache', title: reminderTitle, message, assigned_to: user?.id, service: user?.service }]);
                    }
                }
            }
        };
    ...
    }, [user?.id, user?.role, user?.name, user?.service]);
